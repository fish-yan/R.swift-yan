...
extension Struct {
  func addingInternalProperties(forBundleIdentifier bundleIdentifier: String, hostingBundle: String? = nil) -> Struct {
    ...
    let internalFunctions = [
      ...
      Function(
        availables: [],
        comments: ["get current setting language"],
        accessModifier: .filePrivate,
        isStatic: true,
        name: "getCurrentSetLanguage",
        generics: nil,
        parameters: [],
        doesThrow: false,
        ...
        body: """
            ...
            var currentLanguage = UserDefaults.standard.string(forKey: "userLanguage")
            if currentLanguage == nil || currentLanguage == "" {
